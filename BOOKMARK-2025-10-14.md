...
## Unmatched and Orphans Analysis (October 20, 2025)

### Overview
Analyzed the 2,954 unmatched records from UPDATE_4 enrichment to identify two distinct categories requiring different handling:

1. **Unmatched (62 records)** - Real Mindat IDs but not enriched (likely DISCREDITED/QUESTIONABLE status)
2. **Orphans (225 records)** - No real Mindat IDs (ER placeholders or empty), need character matching and API search

### Unmatched File Generation

**Purpose:** Capture minerals with valid Mindat IDs that were filtered out during enrichment (DISCREDITED/QUESTIONABLE status minerals)

**Script:** `scripts/create-unmatched-file.ts`

**Process:**
1. Load e-Rocks_UPDATE_4_UK.csv (all 9,287 records)
2. Filter to records with:
   - Real Mindat ID (numeric, not ER prefix)
   - No Mindat Status (wasn't enriched in main process)
   - Class = "Mineral" only
3. Query Neon database WITHOUT IMA status filter (include ALL statuses)
4. Enrich with complete Mindat data
5. Apply ALL formatting rules (UK spelling, Unicode, zero-padded strunz)

**Results:**
- Total unmatched with real IDs: 62 records
- ✅ Found in Neon (enriched): 62 (100%)
- ❌ Not found in Neon: 0

**Output:** `/tmp/phase1-enrichment/e-Rocks_UPDATE_4_UK_Unmatched.csv` (16K)
**Test File:** `/tmp/phase1-enrichment/e-Rocks_UPDATE_4_UK_Unmatched_TEST.csv` (10 samples)

### Orphans File Generation

**Purpose:** Find minerals without Mindat IDs using character normalization and API search

**Script:** `scripts/create-orphans-file.ts`

**Process:**
1. Load e-Rocks_UPDATE_4_UK.csv (all 9,287 records)
2. Filter to records with:
   - ER or empty Mindat ID
   - No Mindat Status (wasn't enriched by name match)
   - Class = "Mineral" only
3. Load Neon database into memory (6,196 approved minerals)
4. For each orphan:
   - Try normalized character matching against Neon (removes accents, special chars)
   - If not found, search Mindat API
   - Mark as true orphan if not found anywhere
5. Enrich found minerals with complete Mindat data
6. Apply ALL formatting rules

**Character Normalization Examples:**
- Giuscaite → Giuşcăite ✅
- Nyboite → Nybøite ✅
- Kvacekite → Kvačekite ✅
- Dubinskaite → Dubińskaite ✅
- Kupčikite → Kupčíkite ✅
- Whiteite-CaFeMg → Whiteite-(CaFeMg) ✅

**Results:**
- Total orphans: 225 records
- ✅ Found via character normalization: 22 (9.8%)
- ✅ Found via API search: 0
- ❌ True orphans (not found): 203 (90.2%)

**Output:** `/tmp/phase1-enrichment/e-Rocks_UPDATE_4_Orphans.csv` (44K)
**Test File:** `/tmp/phase1-enrichment/e-Rocks_UPDATE_4_Orphans_TEST.csv` (10 samples: 5 matched + 5 orphans)

### Data Quality Issues Discovered

**Non-Mineral Records with Class="Mineral":**
Many records incorrectly tagged as "Mineral" in source data:
- Lava Bomb, Pearl, DVD, Hardness Scale, Caesium
- Volcanic Memorabilia, Sea Urchin, Fossil items
- These appear in Orphans file because source data has Class="Mineral"
- Require manual review and Class correction in e-rocks

**True Orphans May Include:**
- Misspelled mineral names
- Minerals not yet in Mindat database
- Variety/trade names not recognized by Mindat
- Incorrectly classified non-minerals
- Historical/obsolete mineral names

### Verification Results

**Rule Compliance:**
- ✅ UK spelling: Working (7 records with UK terms)
- ✅ Unicode formulas: 80.9% coverage (38/47)
- ✅ Zero-padded strunz: 87.5% coverage (7/8)
- ✅ Character matching: Working (22 orphans matched)
- ✅ Class filter: Applied (reduced from 736 to 225 orphans)

**Script:** `scripts/verify-rules.ts`

### Next Steps

1. **Import Unmatched File:**
   - Review DISCREDITED minerals for accuracy
   - Consider whether to import or skip based on IMA status
   - May want to flag as DISCREDITED in e-rocks

2. **Review Orphans File:**
   - Manual review of 203 true orphans
   - Correct misspellings where possible
   - Update Class field for non-minerals
   - Re-run character matching after corrections
   - Consider adding to Mindat if legitimate new minerals

3. **Test Character-Matched Orphans:**
   - Import 22 enriched orphans first
   - Verify Mindat IDs are correct
   - Check formulas and classifications
